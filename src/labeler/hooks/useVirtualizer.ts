...
import {
  VIRTUALIZATION_RENDER_AHEAD,
  VIRTUALIZATION_SCROLL_DEBOUNCE,
...
const findStartLineIndex = (
  ...
  while (endRange !== startRange) {
    ...
    if (
      linePositions[middle] <= scrollTop &&
      linePositions[middle + 1] > scrollTop
    ) {
      return middle;
    }
 ...
 */
const findEndLineIndex = (
  linePositions: number[],
  startLine: number,
  lineCount: number,
  labelerHeight: number
) => {
  let endNode;
...
const useScrollTracker = (
  ...
  const onScroll = React.useCallback(
    debounce(
      (e: WheelEvent) => setScrollTop((e.target as HTMLDivElement).scrollTop),
      VIRTUALIZATION_SCROLL_DEBOUNCE
    ),
    []
  );
...
export const useVirtualizer = ({
  ...
  React.useLayoutEffect(() => {
    if (
      !isLabelerMounted ||
      !isVirtualizationEnabled ||
      virtualizationStore.lineHeights.length === 0
    ) {
      return;
    }
    ...
    const firstVisibleLineIndex = findStartLineIndex(
      scrollTop,
      linePositions,
      lineCount
    );
    const lastVisibleLineIndex = findEndLineIndex(
      linePositions,
    ...
    let startLine = Math.max(
      0,
      firstVisibleLineIndex - VIRTUALIZATION_RENDER_AHEAD
    );
    let endLine = Math.min(
      lineCount - 1,
      lastVisibleLineIndex + VIRTUALIZATION_RENDER_AHEAD
    );
    ...
    if (annotationsInRange.length) {
      ...
      const lastLineIndex =
        lastAnnotation.lineSegments[lastAnnotation.lineSegments.length - 1]
          .lineIndex;
    ...
    isVirtualizationEnabled,
    scrollTop,
    virtualizationStore.lineHeights,
    annotationDomData,
  ]);
};
